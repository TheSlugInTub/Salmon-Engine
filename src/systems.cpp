...
void RigidBody3DStartSys()
{
	...
	for (EntityID ent : SceneView<Transform, RigidBody3D>(engineState.scene)) // Loops over all entities with transform and rigidbody components
	{
		...
		if (rigid->colliderType == ColliderType::Box)
		{
			...
			BodyCreationSettings floor_settings(floor_shape, RtransPosition, Quat::sIdentity(), rigid->state == BodyState::Dynamic ? EMotionType::Dynamic : EMotionType::Static, rigid->state == BodyState::Dynamic ? Layers::MOVING : Layers::NON_MOVING);
