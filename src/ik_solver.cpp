...
// end point Calculate the ik.pointDistance between n and n-1 point, calculate
// the direction normalize direction, and put n-1 point in the
// direction + ik.pointDistance Flip the list and do it again
...
void SolveIK2D(IKSolver2D& ik)
{
    ...
    for (int i = ik.points.size() - 1; i > 0; --i)
    {
        ...
        glm::vec2 direction = glm::normalize(ik.points[i - 1] - point);
